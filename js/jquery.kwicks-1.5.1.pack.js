	...
	for(i=0;i<k.size();i++)
		...
		for(j=1;j<k.size()-1;j++){
			if(i==j)
				...
				{m[i][j]=(j<=i?(j*o.min):(j-1)*o.min+o.max)+(j*o.spacing)}}}k.each(function(i){var h=$(this);
				if(i===0){h.css(r,'0px')}
				else 
				if(i==k.size()-1)
					{h.css(o.isVertical?'bottom':'right','0px')}
				else{
					if(o.sticky){h.css(r,m[o.defaultKwick][i])}
					else{
						...
						if(o.defaultKwick==i){
							...
							h.addClass('active')
								...
								h.bind(o.event,function(){var c=[];var d=[];
										...
										if(j!=i){
											...
											if(j>0&&j<k.size()-1){
												...
												for(i=0;i<k.size();i++)
												{
													...
													e[q]=l;
													var f=l-c[0];
													k.eq(0).animate(e,{step:function(a){var b=f!=0?(a-c[0])/f:1;
													for(i=1;i<k.size();i++)
													{
